...
export abstract class BaseWebpart<T> extends BaseClientSideWebPart<T> {
  ...
  protected async onInit(): Promise<void> {
    ...
    if (this.isRunningInDev()) await this.setupForWorkbench();

    if (
      this.config &&
      this.config.loadJSOM &&
      !this.isRunningInLocalWorkbench()
    )
      await this.loadJSOM();
  ...
  private async loadJSOM(): Promise<void> {
    try {
      ...
      if (!(window as any).initJsLoaded) {
        await SPComponentLoader.loadScript(
          siteColUrl + "/_layouts/15/init.js",
          {
